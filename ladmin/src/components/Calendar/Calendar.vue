  ...
  <div class="calendar">
    <div class="swiper-container">
      <div class="calendar-tools">
        ...
        <div
          style="display: flex;justify-content: space-around;line-height: 40px;height: 30px;width: 100px;"
        >
          <span
      ...
      <div>
        <table cellpadding="5">
          <thead>
            <tr>
              <td :key="week" v-for="week in weeks" class="week">{{ week }}</td>
            </tr>
          </thead>
        </table>
      </div>
      <!-- <div class="swiper-wrapper">
                <div :key="n" v-for="n in totalMonth" class="swiper-slide"> -->
      <table cellpadding="5">
        <tbody>
          <tr
            :key="day"
  ...
  methods: {
    ...
    render(y, m) {
      ...
      for (i = 1; i <= lastDateOfMonth; i++) {
        ...
        if (this.value.length > 0) {
          if (
            seletSplit[0] == this.year &&
            seletSplit[1] - 1 == this.month &&
            i == seletSplit[2]
          ) {
        ...
        for (let j = 0; j < seletSplitS.length; j++) {
          if (
            parseInt(seletSplitS[j][0]) == this.year &&
            parseInt(seletSplitS[j][1]) - 1 == this.month &&
            parseInt(seletSplitS[j][2]) == i
          ) {
            // 选中相应的天
